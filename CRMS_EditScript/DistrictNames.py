...
def DNames(textDN):
    text01_DN = textDN.replace('NORTHEASTC-Code Compliance Sub-Districts (SW NCSCNWSENECE - ABC) NORTHEAST-Code Compliance District ','NE_SubC,')
    text02_DN = text01_DN.replace('NORTHEAST-Code Compliance District ','NE,')
    text03_DN = text02_DN.replace('SOUTHWEST-Code Compliance District ','SW,')
    

# Multi-Tenant District areas

    text01_MT = text03_DN.replace('WEST-Multi Tenant District ','W-MTD,')
    text02_MT = text01_MT.replace('EAST-Multi Tenant District ','E-MTD,')

# Inspector Areas
    text01_IA = text02_MT.replace('SOUTHWEST-Code Inspector Area ','SW-InspArea,')
    text02_IA = text01_IA.replace('NORTHEAST-Code Inspector Area ','NE-InspArea,')
    text03_IA = text02_IA.replace('-Code Inspector Area ','-InspArea,')
